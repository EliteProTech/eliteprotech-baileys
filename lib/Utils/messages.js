...
const prepareStickerPackMessage = async (message, options) => {
    const { cover, stickers = [], name = '📦 Sticker Pack', publisher = 'EliteProTech', description = 'EliteProTech Sticker Pack' } = message;
    ...
    const logger = options.logger;
    const jimpModule = await Promise.resolve().then(() => require('jimp')).catch(() => null);
    const Jimp = jimpModule?.Jimp || jimpModule?.default || jimpModule;
    if (!Jimp) {
        throw new boom_1.Boom('Jimp is required for sticker pack image conversion', { statusCode: 400 });
    }
    ...
    const toBuffer = async media => {
        const { stream } = await messages_media_1.getStream(media);
        return await messages_media_1.toBuffer(stream);
    };
    const stickerData = {};
    const stickerMetadata = new Array(stickers.length);
    for (let i = 0; i < stickers.length; i++) {
        const item = stickers[i] || {};
        const media = item.data || item.sticker || item;
        let webpBuffer = await toWebP(await toBuffer(media));
        if (webpBuffer.length > 1024 * 1024) {
            throw new boom_1.Boom(`Sticker at index ${i} exceeds the 1MB size limit`, { statusCode: 400 });
        }
        const hash = crypto_1.createHash('sha256').update(webpBuffer).digest('base64').replace(/\//g, '-');
    ...
    const stickerPackId = generics_1.generateMessageIDV2();
    const coverBuffer = await toWebP(await toBuffer(cover));
    const trayIconFileName = `${stickerPackId}.webp`;
    stickerData[trayIconFileName] = [new Uint8Array(coverBuffer), { level: 0 }];
    const zipBuffer = Buffer.from((0, fflate_1.zipSync)(stickerData));
    ...
    let uploadResult;
    try {
        uploadResult = await options.upload(stickerPackUpload.encFilePath, {
        ...
        fs_1.promises.unlink(stickerPackUpload.encFilePath).catch(() => logger?.warn?.('failed to remove sticker pack tmp file'));
    }
    const obj = {
        name,
        publisher,
        ...
        stickerPackOrigin: Types_1.WAProto.Message.StickerPackMessage.StickerPackOrigin.USER_CREATED,
    ...
    try {
        ...
        coverImage.resize({ w: 252, h: 252 });
        const thumbnailBuffer = await coverImage.getBuffer('image/jpeg');
        ...
        try {
            ...
            fs_1.promises.unlink(thumbUpload.encFilePath).catch(() => logger?.warn?.('failed to remove sticker thumbnail tmp file'));
        }
        Object.assign(obj, {
            ...
            thumbnailHeight: 252,
            thumbnailWidth: 252,
            imageDataHash: crypto_1.createHash('sha256').update(thumbnailBuffer).digest('base64')
        });
    }
    catch (error) {
        logger?.warn?.(`Sticker pack thumbnail generation failed: ${error}`);
    }
    return Types_1.WAProto.Message.StickerPackMessage.fromObject(obj);
};
...
const generateWAMessageContent = async (message, options) => {
    ...
    else if ('stickerPackMessage' in message) {
        m.stickerPackMessage = await prepareStickerPackMessage(message.stickerPackMessage, options);
    }
