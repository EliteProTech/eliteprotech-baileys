...
const generateWAMessageContent = async (message, options) => {
    ...
    const { stickers, cover, name = '📦 Sticker Pack', publisher = 'EliteProTech', packId, description = 'EliteProTech Sticker Pack' } = message.stickerPack;
    ...
    const isWebPBuffer = buffer => Buffer.isBuffer(buffer) && buffer.length >= 12 && buffer.slice(0, 4).toString() === 'RIFF' && buffer.slice(8, 12).toString() === 'WEBP';
    const isAnimatedWebP = buffer => {
        let offset = 12;
        while (offset + 8 <= buffer.length) {
            const fourCC = buffer.toString('ascii', offset, offset + 4);
            const chunkSize = buffer.readUInt32LE(offset + 4);
            if (fourCC === 'VP8X' && offset + 8 < buffer.length && (buffer[offset + 8] & 0x02)) return true;
    ...
    const toWebP = async buffer => {
        ...
        try {
            const sharpModule = await Promise.resolve().then(() => require('sharp')).catch(() => null);
            const sharp = sharpModule?.default || sharpModule;
            if (sharp) return await sharp(buffer).resize(512, 512, { fit: 'contain' }).webp({ quality: 85 }).toBuffer();
        } catch (_) {}
        const id = crypto.randomBytes(8).toString('hex');
        const inputPath = path.join(os.tmpdir(), `sticker-pack-${id}-input`);
        const outputPath = path.join(os.tmpdir(), `sticker-pack-${id}.webp`);
        try {
            await fs.promises.writeFile(inputPath, buffer);
            await promisify(execFile)('ffmpeg', ['-y', '-i', inputPath, '-vf', 'scale=512:512:force_original_aspect_ratio=decrease,pad=512:512:(ow-iw)/2:(oh-ih)/2:color=0x00000000', '-c:v', 'libwebp', '-q:v', '85', outputPath]);
            return await fs.promises.readFile(outputPath);
        } catch (error) {
            throw new boom_1.Boom(`Sticker pack image conversion failed: ${error.message || error}`, { statusCode: 400 });
        } finally {
            await fs.promises.unlink(inputPath).catch(() => {});
            await fs.promises.unlink(outputPath).catch(() => {});
        }
    };
    const stickerMetadata = await Promise.all(stickers.map(async (item, index) => {
        const stickerInput = item?.sticker ?? item?.data ?? item;
        if (!stickerInput) throw new boom_1.Boom(`Sticker pack: sticker at index ${index} is missing its media`, { statusCode: 400 });
        const { stream } = await (0, messages_media_1.getStream)(stickerInput, options.options);
        const sourceBuffer = await (0, messages_media_1.toBuffer)(stream);
        if (!sourceBuffer || sourceBuffer.length === 0) throw new boom_1.Boom(`Sticker pack: sticker at index ${index} is empty`, { statusCode: 400 });
        const buffer = await toWebP(sourceBuffer);
        if (!isWebPBuffer(buffer)) throw new boom_1.Boom(`Sticker pack: sticker at index ${index} could not be converted to WebP`, { statusCode: 400 });
        if (buffer.length > 1024 * 1024) {
            throw new boom_1.Boom(`Sticker pack: sticker at index ${index} exceeds the 1MB size limit`, { statusCode: 400 });
        }
        const hash = (0, crypto_1.createHash)('sha256').update(buffer).digest('base64').replace(/\//g, '-');
        const fileName = `${index.toString().padStart(2, '0')}_${hash}.webp`;
        stickerData[fileName] = [new Uint8Array(buffer), { level: 6 }];
        return {
            fileName,
            mimetype: 'image/webp',
            isAnimated: typeof item?.isAnimated === 'boolean' ? item.isAnimated : isAnimatedWebP(buffer),
            isLottie: !!item?.isLottie,
            emojis: Array.isArray(item?.emojis) && item.emojis.length ? item.emojis : ['✨'],
            accessibilityLabel: item?.accessibilityLabel || ''
        };
    }));
    const coverBufferSource = await messages_media_1.toBuffer((await messages_media_1.getStream(cover, options.options)).stream);
    if (!coverBufferSource || coverBufferSource.length === 0) throw new boom_1.Boom('Sticker pack cover is empty', { statusCode: 400 });
    const coverBuffer = await toWebP(coverBufferSource);
    if (!isWebPBuffer(coverBuffer)) throw new boom_1.Boom('Sticker pack cover could not be converted to WebP', { statusCode: 400 });
    const stickerPackId = packId || (0, generics_1.generateMessageIDV2)();
    const trayIconFileName = `${stickerPackId}.webp`;
    stickerData[trayIconFileName] = [new Uint8Array(coverBuffer), { level: 6 }];
    ...
    try {
        ...
        fs_1.promises.unlink(stickerPackUpload.encFilePath).catch(() => options.logger?.warn?.('failed to remove sticker pack tmp file'));
    }
    const coverImage = coverUpload.imageMessage;
    m.stickerPackMessage = Types_1.WAProto.Message.StickerPackMessage.fromObject({
        name,
        publisher,
        stickerPackId,
        ...
        mediaKeyTimestamp: (0, generics_1.unixTimestampSeconds)(),
        trayIconFileName,
        imageDataHash: (0, crypto_1.createHash)('sha256').update(coverBuffer).digest('base64'),
