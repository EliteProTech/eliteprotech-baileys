...
const generateWAMessageContent = async (message, options) => {
    ...
    const stickerMetadata = await Promise.all(stickers.map(async (item, index) => {
        ...
        if (!stickerInput) {
            throw new boom_1.Boom(`Sticker pack: sticker at index ${index} is missing its media`, { statusCode: 400 });
        }
        ...
        if (!buffer || buffer.length === 0) {
            throw new boom_1.Boom(`Sticker pack: sticker at index ${index} is empty`, { statusCode: 400 });
        }
        if (!isWebPBuffer(buffer)) {
            throw new boom_1.Boom(`Sticker pack: sticker at index ${index} is not a valid WebP image`, { statusCode: 400 });
        }
    ...
    const coverBuffer = await messages_media_1.toBuffer((await messages_media_1.getStream(cover, options.options)).stream);
    if (!coverBuffer || coverBuffer.length === 0) {
        throw new boom_1.Boom('Sticker pack cover is empty', { statusCode: 400 });
    }
    if (!isWebPBuffer(coverBuffer)) {
        throw new boom_1.Boom('Sticker pack cover must be a valid WebP image', { statusCode: 400 });
    }
