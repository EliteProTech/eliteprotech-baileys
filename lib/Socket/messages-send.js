...
const makeMessagesSocket = (config) => {
    ...
    const getMediaType = (message) => {
        if (message.imageMessage) {
            ...
            if (innerMsg.stickerPackMessage) return "sticker_pack";
            return "text";
        }
    };
    ...
    return {
        ...
        sendStickerPack: async (jid, images, options = {}, sendOptions = {}) => {
            if (!Array.isArray(images) || images.length === 0) {
                throw new Error("Sticker pack requires at least one image")
            }
            const {
                name = "EliteProTech",
            ...
            const { quoted } = sendOptions
            const generated = await (0, Utils_1.generateWAMessage)(
                jid,
                {
                    stickerPack: {
                        stickers: images.map(image => ({ data: image })),
                        cover: images[0],
                        name,
                        publisher,
                        packId: `eliteprotech-${Date.now()}`,
                        description
                    }
                },
                {
            ...
            const pack = generated.message?.stickerPackMessage
            if (!pack) {
                throw new Error("StickerPackMessage was not generated")
            }
            await relayMessage(
                jid,
