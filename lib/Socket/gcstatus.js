...
class GiftedStatus {
    ...
    async sendGroupStatus(groupJid, content, options = {}) {
        ...
        if (content.message) {
            ...
            const generated = await Utils_1.generateWAMessage(groupJid, messageContent, {
            ...
            waMsgContent = generated.message;
        }
    ...
    async handleGroupStory(content, jid, quoted) {
        ...
        if (storyData.message) {
            ...
            const userJid = WABinary_1.jidNormalizedUser(this.sock.authState.creds.me.id);
            const isMedia = !!(storyData.image || storyData.video || storyData.audio);
            const isAudio = !!storyData.audio;
            const messageContent = { ...storyData };
            ...
            const font = !isMedia ? (storyData.font ?? Math.floor(Math.random() * 9)) : undefined;
            const textColor = !isMedia ? (storyData.textColor || getRandomHexColor()) : undefined;
            const backgroundColor = (!isMedia || isAudio) ? (storyData.backgroundColor || getRandomHexColor()) : undefined;
            const ptt = isAudio ? (typeof storyData.ptt === 'boolean' ? storyData.ptt : true) : undefined;
            const link_preview_1 = require("../Utils/link-preview");
